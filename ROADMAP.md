...
Ariel OS is developed and maintained by a non-profit community of open-source developers organized around principles that are inspired by the Linux community or the IETF community among others.
...
We are working on extending support for additional network link-layers (BLE, 802.15.4, LoRa) and for additional open standard network protocols (MQTT, DTLS...) to facilitate integration in various backends (Home Assistant...).
