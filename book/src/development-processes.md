...
1. Update the changelog using [changelog-harvester](https://codeberg.org/AudaciousAxiom/changelog-harvester) and the following command (see its documentation for details):
   ```sh
   changelog-harvester --with-token --forge-owner 'ariel-os' --forge-repo 'ariel-os' --labels 'changelog:highlight,breaking,changelog:new-hardware,changelog:sensor,changelog:skip' --old '<previous-version>' --new '<new-version>' < ../token.txt | sed 's|(#\([[:digit:]]\+\))$|([#\1](https://github.com/ariel-os/ariel-os/pull/\1))|'
   ```
