...
Feel free to report anything that is unclear!
...
Most MCU-specific items inside an Ariel OS HAL crate gated behind
`#[cfg(context = $CONTEXT)]` attributes, where `$CONTEXT` is the MCU's `laze
context`.
These need to be expanded for adding support to the new MCU.
