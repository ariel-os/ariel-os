...
fn validate_input(matrix: &schema::Matrix) -> Result<(), Error> {
    ...
    let mut errors = board_errors.chain(chip_errors).collect::<Vec<_>>();
    ...
    if errors.is_empty() {
        ...
        Err(Error::MultipleIssues { others: errors })
    }
}
...
enum Error {
    ...
    #[error("Multiple validation issues")]
    MultipleIssues {
        #[related]
        others: Vec<Error>,
    },
