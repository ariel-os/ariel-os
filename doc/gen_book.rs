    ...
    let boards = matrix
        ...
        .map(|(board_technical_name, board_info)| {
            ...
            let functionalities = matrix.functionalities.iter().map(|functionality_info| {
                let support_key = if functionality_info.support_criteria.iter().all(|c| match c {
                    schema::SupportCriteria::AppOnly(app) => {
                        laze_info.builds[board_technical_name].get(app).is_some()
                    }
                    schema::SupportCriteria::WithModule(app_with_module) => {
                        let (app, module) = app_with_module.into_iter().next().unwrap();
                        laze_info.builds[board_technical_name]
                            .get(app)
                            .is_some_and(|app| app.modules.get(module).is_some())
                    }
                }) {
                    matrix
                        .support_keys
