...
use ariel_os_sensors::{
    ...
    sensor::{
        ...
        ReadingWaiter, Sample, SampleMetadata, Samples, SetModeError, State,
...
use ariel_os_debug_log::{Debug2Format, Hex, debug};

const I2C_ADRESS: u8 = 0x29;

use crate::{AlsGain, AlsIntegrationTimeMs, AlsMeasurementRateMs, MeasuredAlsDataGain, Register};
use crate::{MANU_ID, PART_ID, PART_NUMBER, REV_ID};
...
impl<I2C: I2c + Send> Ltr303Als01<I2C> {
    ...
    pub async fn init(
        ...
        if !self.i2c.is_set() {
            ...
            if i2c_device
                .write(I2C_ADRESS, &[Register::GainCtrl as u8, gain_ctrl])
                .await
                .is_err()
            {
                return;
            ...
            // TODO: Support the usage of interrupts
            // let mut interrupt_mode: u8 = 0;
            // if config.interrupt_enable {
            //     interrupt_mode |= crate::INTERRUPT_MODE;
            // }
            // if config.interrupt_polarity {
            //     interrupt_mode |= crate::INTERRUPT_POLARITY;
            // }
    ...
    pub async fn set_config(&self, config: Config) -> Result<(), ()> {
        ...
        i2c.write(I2C_ADRESS, &[Register::GainCtrl as u8, gain_ctrl])
            .await
            .map_err(|_| ())?;
    ...
    pub async fn measure(&'static self) -> ReadingResult<Samples> {
        ...
        let mut buf = [0u8];
        i2c.write_read(I2C_ADRESS, &[Register::GainCtrl as u8], &mut buf)
            .await
