    ...
    pub async fn init(
        ...
        if !self.i2c.is_set() {
            ...
            loop {
                ...
                debug!("[AHT20 Init] Status register: {:#010b}", buf[0]);
    ...
    async fn measure(&'static self) -> ReadingResult<Samples> {
        ...
        loop {
            match i2c.read(I2C_ADDRESS, &mut buf).await {
                ...
                Ok(()) => {
                    debug!("[AHT20 Measuring] status register: {:#010b}", buf[0]);
        ...
        debug!(
            "[AHT20 Measuring] raw data: {:#012X}",
            u64::from_be_bytes([0, 0, 0, buf[1], buf[2], buf[3], buf[4], buf[5]])
        );
        debug!("[AHT20 Measuring] CRC: {:#04X}", buf[6]);
        ...
        // Bitshift to only get the value from the 20 most significant bits of buf[1..4].
        let humidity = i32::from_be_bytes([0, buf[1], buf[2], buf[3]]) >> 4;
        // Masking the 4 most significant bits of buf[3] to get the proper value.
        let temperature = i32::from_be_bytes([0, 0x0F & buf[3], buf[4], buf[5]]);

        // H = (Sh / 2^20) * 100 % see Section 6.1 of the Datasheet.
        // This naturally brings the resolution down the %.
        // This can't overflow because there are 11 bits of extra space and x100 < x128 = << 7.
        let humi = (humidity * 100) >> 20;

        // T = (St / 2^20) * 200 - 50 see Section 6.2 of the Datasheet.
        // We pre-multiply by 10 to keep the decimal digit.
        // This can't overflow because there are 11 bits of extra space and x2000 < x2048 = << 11.
